...
xvawaeme = ["api16-va.tiktokv.com", "api15-va.tiktokv.com", "api19-va.tiktokv.com", "api.tiktokv.com"]
...
def send(did, iid, cdid, openudid):
    ...
    for x in range(1):
        try:
            ...
            xpcpdomn = random.choice(xvawaeme)

            ctypes.windll.kernel32.SetConsoleTitleW(F"S:{success} F:{fails} R:{reqs}")

            proxy = random.choice(proxies) if use_proxy else ""
            response = requests.post(
                url=(
                    f"https://{xpcpdomn}/aweme/v1/aweme/stats/?" + params
                ),
                data=payload,
                headers={'cookie': 'sessionid=0e80e7db81b31e3fdac509d3a639bff3', 'x-gorgon': sig['X-Gorgon'],
                         'x-khronos': sig['X-Khronos'], 'user-agent': 'okhttp/3.10.0.1'},
                verify=False,
                proxies={"http": proxy_format + proxy, "https": proxy_format + proxy} if use_proxy else {}
            )
            reqs += 1
            response_data = (response.text)
            try:
                json_data = json.loads(response_data)
                status_code = json_data.get("status_code", -1)
